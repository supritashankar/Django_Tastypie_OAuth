...
class Consumer(models.Model):
    ...
    key = models.CharField(max_length=CONSUMER_KEY_SIZE, editable=False)
    secret = models.CharField(max_length=SECRET_SIZE, blank=True, editable=False)

    status = models.SmallIntegerField(choices=CONSUMER_STATES, default=PENDING)
    user = models.ForeignKey(User, null=True, blank=True)
        
    def __unicode__(self):
        return u"Consumer %s with key %s and secret %s" % (self.name, self.key, self.secret)
